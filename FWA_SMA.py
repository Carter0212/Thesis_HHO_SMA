...
import pickle
# W = (10**9) ##(Hz)Thesis_HHO_SMA/FWA_T1.py
# alpha=2 ##(Path loss exponent)
# lambd = 0.005 #(m)
Min_Rate = 6*(10**8) #(bit/s)
# N_0=-174 #(dBm/Hz)
# Min_Power = 0 #(mW)
Max_power = 1000 #(mW)
# base_numbers = 4
...
def find_MaxEE(X,BS_FWA_distance = False):
    ...
    # print(check_conectional)
    # result_array = np.zeros((A, B), dtype=bool)
    
    max_indices = np.argmax(NP_three_D_X[0], axis=0)
    ...
    if np.all((np.sum(check_conectional,axis=0) > 0) & (np.sum(check_conectional,axis=0) < 2)):
        Check_constrained[3] = True
    # print(np.sum(check_conectional,axis=0))    
    Zero_mask_connect_UE=np.where((np.sum(check_conectional,axis=0) <= 0) & (np.sum(check_conectional,axis=0) >= 2))
    ...
    Base_ue_ThroughtpusTable=transmit_bandwidth * np.log2(1+(check_conectional*BS_FWA_distance*(NP_three_D_X[1])))
    ...
    constrained_violation = (
        ...
        np.sum(np.maximum( np.abs(1-np.sum(check_conectional,axis=0)) ,0) )*10000 +   
    ...
    test_num = ((not Check_constrained[0])*10000,
                (not Check_constrained[1])*10000,
                np.sum(np.maximum( np.abs(1-np.sum(check_conectional,axis=0)) ,0) ),   
...
def PreProcesing():
    ...
    # 32.4+21*math.log10(Distance)+20*math.log10(carrier_frequency)
    N0=-83.02 #dBm
...
if __name__ == "__main__":
    ...
    carrier_frequency = 28 # GHz
    N0=-83.02 #dBm
    # with open(f'{CR}_BS','w',newline='') as file_BS:
    #     writer_BS = csv.writer(file_BS)
    #     writer_BS.writerows(bs_positions)
    # with open(f'{CR}_UE','w',newline='') as file_UE:
    #     writer_UE = csv.writer(file_UE)
    #     writer_UE.writerows(ue_positions)
    ...
    # Ga1.save_fitness_params(BS_FWA_distance,carrier_frequency,Antenna_gain,LOS_shadow,NLOS_shadow,N0,transmit_bandwidth)
    ...
    # hho_1.save_fitness_params(BS_FWA_distance,carrier_frequency,Antenna_gain,LOS_shadow,NLOS_shadow,N0,transmit_bandwidth)
    ...
    # hhoSMA_chaos_1.save_fitness_params(BS_FWA_distance,carrier_frequency,Antenna_gain,LOS_shadow,NLOS_shadow,N0,transmit_bandwidth)
    ...
    OriginalSMA_1.save_fitness_params(BS_FWA_distance,carrier_frequency,Antenna_gain,LOS_shadow,NLOS_shadow,N0,transmit_bandwidth)
